...
class PointMassEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.box4_x_y_length = np.array([10, 5, 1])  # Goal is at x=10, y=6, 1 unit square length (squares expand to higher coordinates)
  ...
  def step(self, action):
    ...
    if self.grid_cells is None:
      ...
      g_x = self.safety_margin(self.scaling, self.state, safety_radius, cutoff_radius, safety_threshold, local_map)
    else:
      nearest_point = nearest_real_grid_point(
          self.grid_cells, self.bounds, self.state
      )
      l_x = self.target_margin(nearest_point)
      g_x = self.safety_margin(self.scaling, nearest_point, safety_radius, cutoff_radius, safety_threshold, local_map)
  ...
  def simulate_one_trajectory(self, q_func, T=10, state=None):
    ...
    for t in range(T):
      ...
      if done:
        break
      '''
            if safety_margin(self.scaling, state, safety_radius, cutoff_radius, safety_threshold, local_map) > 0 or self.target_margin(state) < 0:
                break
            '''
