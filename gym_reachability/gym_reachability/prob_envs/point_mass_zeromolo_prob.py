  ...
  def __init__(
    ...
    if envType == 'basic' or envType == 'easy':
      self.visual_initial_states = [
          np.append([1.8, 0.0], conv_grid(cur_pos=[1.8, 0.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
          np.append([8.0, 0.0], conv_grid(cur_pos=[8.0, 0.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
          np.append([4.5, 1.1], conv_grid(cur_pos=[4.5, 1.1], filter=self.filter, R=self.conv_radius, stride=self.stride)),
          np.append([8.0, 5.0], conv_grid(cur_pos=[8.0, 5.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
      ]
