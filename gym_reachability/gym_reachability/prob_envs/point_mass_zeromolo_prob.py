...
class ProbZermeloShowEnv(gym.Env):
  ...
  def __init__(
    ...
    if envType == 'basic' or envType == 'easy':
      self.target_x_y_w_h = np.array([[4.5, 9, 4, 0.8]])
    else:
      self.target_x_y_w_h = np.array([[4.5, 9, 4, 0.8]])
    ...
    if envType == 'basic' or envType == 'easy':
      self.visual_initial_states = [
          np.append([1.8, 1.0], conv_grid(cur_pos=[1.8, 1.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
          np.append([8.0, 1.0], conv_grid(cur_pos=[8.0, 1.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
