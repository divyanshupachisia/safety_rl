...
class ProbZermeloShowEnvCurvy(gym.Env):
    ...
    def __init__(
        ...
        # Local Map
        self.local_map = np.transpose(gen_grid(type="curvy"))
        ...
        self.width_conv_filter_dimension = math.ceil(
            ((2 * self.conv_radius + 1) - len(self.filter) + 1) / self.stride)  # TODO ask Divy to check this
        self.height_conv_filter_dimension = math.ceil(
            ((2 * self.conv_radius + 1) - len(self.filter[0]) + 1) / self.stride)  # TODO ask Divy to check this

        self.conv_states_count = self.width_conv_filter_dimension * self.height_conv_filter_dimension

        assert (len(conv_grid(cur_pos=[0, 0], filter=self.filter, R=self.conv_radius,
                              stride=self.stride)) == self.conv_states_count)  # TODO remove this
        ...
        if envType == 'basic' or envType == 'easy':
            self.visual_initial_states = [
                np.append([4.0, 1.0],
                          conv_grid(cur_pos=[4.0, 1.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
                np.append([4.0, 2.0],
                          conv_grid(cur_pos=[4.0, 2.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
                np.append([3.0, 3.0],
                          conv_grid(cur_pos=[3.0, 3.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
                np.append([2.0, 4.0],
                          conv_grid(cur_pos=[2.0, 4.0], filter=self.filter, R=self.conv_radius, stride=self.stride)),
            ]
    ...
    def sample_random_state(self, sample_inside_obs=False):
        ...
        while inside_obs:
            ...
            new_state = np.append(xy_sample, conv_grid(cur_pos=xy_sample, filter=self.filter, R=self.conv_radius,
                                                       stride=self.stride))
        ...
        for i in np.arange(og_x - cutoff_radius, og_x + cutoff_radius + (1 / divisions), 1 / divisions):
            for j in np.arange(og_y - cutoff_radius, og_y + cutoff_radius + (1 / divisions), 1 / divisions):
                ...
                grid_distance = math.sqrt(
                    float(((closest_x - i) * (closest_x - i)) + ((closest_y - j) * (closest_y - j))))
                ...
                if (
                        closest_x != closest_i or closest_y != closest_j) and actual_distance <= cutoff_radius:  # Exclude the original cell and all cells farther from the distance

                    square_count += 1

                    # Inside the boundary
                    if closest_i >= 0 and closest_i < len(local_map) and closest_j >= 0 and closest_j < len(
                            local_map[0]):
                        safety_margin = safety_margin + (beta * 1 / (actual_distance + (beta / threshold))) * \
                                        local_map[closest_i][closest_j] / (divisions * divisions)
